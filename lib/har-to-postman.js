...
const generateItemRequestHeaders = (harRequestHeaders, requestHasBody) => {
  if (!requestHasBody) {
    return harRequestHeaders.filter(isNotContentTypeHeader).filter(isRelevantHeader).map(renameHeaderKey);
  } else {
    return harRequestHeaders.filter(isRelevantHeader).map(renameHeaderKey);
  }
};

const isNotContentTypeHeader = (header) => {
  return 'content-type' != header.name.toLowerCase();
}
