...
var generatePostmanItemEvent = function (response, lastPathObject) {
    ...
    if (response.content != null && parseInt(JSON.parse(response.content.text).id) === parseInt(lastPathObject)) {
        scriptExec.push('pm.test("Status code is Fetched object should be the expected one", function () {', '    var jsonData = pm.response.json();    pm.expect(jsonData.id).to.eql(' + lastPathObject + ');', '});');
    }
